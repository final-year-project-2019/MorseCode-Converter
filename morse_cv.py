...
class Detectmorse():
    ...
    def __init__(self):
        ...
        self.dot = 10
        self.dash = 40
        self.detect = dlib.get_frontal_face_detector()    # This detector is based on histogram of oriented gradients (HOG) and linear SVM 
    ...
    def calculate(self,frame):
        frame = imutils.resize(frame, width=640) #resize of image to the corresponding value
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) #Converts an image from one color space to another
        ...
        for subject in subjects:
            ...
            leftEyeHull = cv2.convexHull(leftEye) #draw the hull around the eye
            rightEyeHull = cv2.convexHull(rightEye)
            cv2.drawContours(frame, [leftEyeHull], -1, (0, 255, 0), 1) #contour drawing around the left and right eye
        ...
        if self.openEye > 50:
            ...
            if self.str == None:
                self.L = []
            self.L = []
        cv2.putText(frame, "Predicted :  " + self.final, (10, 470),cv2.FONT_HERSHEY_DUPLEX, 0.7, (52, 152, 219), 2)
